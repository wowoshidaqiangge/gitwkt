<template>
  <div class="chartContent">
    <div class="chart" id="barLineWorkprocess"></div>
  </div>
</template>
<script>
import { workprocessQuality } from '@/api/globalPage';
...
export default {
  ...
  mounted() {
    this.barLineWorkprocessFunc();
  },
  created() {},
  computed: {},
  methods: {
    barLineWorkprocessFunc() {
      workprocessQuality()
        .then(res => {
          console.log('左2', res.data);
          // TODO:空数据
          ...
          for (let item of res.data) {
            ...
            yarr.push(item.qualifiedCount);
            yarr2.push(parseFloat(item.qualifiedRate));
            // yarrTotal.push(item.produceTotal)
          }
          console.log(xarr, yarr, yarr2);
          let myChart = echarts.init(document.getElementById('barLineWorkprocess'));
          let option = {
            color: ['#00B7EE', '#F19149'],
            textStyle: {
              color: '#fff'
